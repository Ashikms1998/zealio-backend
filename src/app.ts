  ...
  .then(() => {
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("webrtcSignal", async (data) => {
        if (data.isCaller) {
          if (
            data.ongoingCall.participants.receiver.id ||
            data.ongoingCall.participants.receiver._id
          ) {
            const emitSocketId = userIdSocketIdMap.get(
              data.ongoingCall.participants.receiver.id
            );
          ...
          if (
            data.ongoingCall.participants.caller.id ||
            ...
            console.log(
              "emitted to caller web",
              emitSocketId,
              "this is emitetr socket",
              userIdSocketIdMap,
      ...
      socket.on("hangup", async (ongoingCall) => {
        try {
          ...
          if (participants && participants.caller && participants.receiver) {
            ...
            console.log(
              `Call ended between ${participants.caller.id} and ${participants.receiver.id}`
            );
          } else {
