...
export class authService implements IUserAuth {
    ...
    async registerUser(input:User,type?:string){
        ...
        if(type){
            ...
            const html = emailTemplate(data.firstName,data.lastName ?? '',email,token);
    ...
    async changePassword(email: string, password: string): Promise<User | null> {
        ...
        console.log(hashedPassword,"hash aakki vro")
        let changeingPasscode = await this.repository.changePasscode(user.id,hashedPassword);
